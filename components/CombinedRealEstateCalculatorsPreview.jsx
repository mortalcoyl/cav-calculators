...
const RETIREMENT_TAX_FILING_STATUSES = [
  { id: "single", label: "Single", brackets: RETIREMENT_TAX_BRACKETS_2025_SINGLE },
  { id: "married-joint", label: "Married filing jointly", brackets: RETIREMENT_TAX_BRACKETS_2025_MARRIED_JOINT },
];
...
function useCalculatorState(calculatorId, field, defaultValue) {
  return usePersistentState(`${STORAGE_PREFIX}:${calculatorId}:${field}`, defaultValue);
}
...
function getRetirementTaxBracket2025(spending, filingStatus = "single") {
  const status = RETIREMENT_TAX_FILING_STATUSES.find((item) => item.id === filingStatus) || RETIREMENT_TAX_FILING_STATUSES[0];
  const bracket = status.brackets.find((item) => spending >= item.min && spending <= item.max) || status.brackets[status.brackets.length - 1];
  return {
    ...bracket,
    filingStatus: status.id,
    filingStatusLabel: status.label,
    label: bracket.max === Infinity ? `${bracket.rate}% over ${formatMoney(bracket.min - 1)}` : `${bracket.rate}% for ${formatMoney(bracket.min)}-${formatMoney(bracket.max)}`,
  };
}
...
function RangeInput({ label, value, onChange, min, max, step = 1, suffix = "", helperText }) {
  ...
  return <div><div className="flex items-center justify-between gap-3"><label className="text-sm font-medium text-neutral-800">{label}</label><div className="flex min-w-[96px] items-center justify-end rounded-xl border border-neutral-200 bg-white px-3 py-1.5 focus-within:border-neutral-950"><input value={draft} onFocus={(event) => event.target.select()} onChange={(event) => setDraft(event.target.value)} onBlur={() => setSafeValue(draft)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} className="w-full bg-transparent text-right text-sm font-semibold outline-none" inputMode="numeric" />{suffix && <span className="ml-1 text-neutral-400">{suffix}</span>}</div></div>{helperText && <p className="mt-1 text-xs leading-5 text-neutral-500">{helperText}</p>}<input type="range" min={min} max={max} step={step} value={value} onChange={(event) => setSafeValue(event.target.value)} className="mt-2 w-full accent-neutral-950" /></div>;
}
...
function SmallStat({ label, value, tone = "neutral", detail }) {
  const toneClass = tone === "green" ? "border-emerald-200 bg-emerald-50" : tone === "blue" ? "border-sky-200 bg-sky-50" : tone === "amber" ? "border-amber-200 bg-amber-50" : tone === "red" ? "border-red-200 bg-red-50" : tone === "teal" ? "border-teal-200 bg-teal-50" : tone === "magenta" ? "border-fuchsia-200 bg-fuchsia-50" : "border-neutral-200 bg-neutral-50";
...
function calculateRentVsBuyScenario({ homePrice, downPaymentPct, rate, loanYears, rent, monthlyCashBeforeHousing, appreciation, marketReturn, rentInflation, propertyTaxPct, annualInsurance, maintenancePct, renovations, buyingClosingCostPct, transferTaxPct = 0, recordationTaxPct = 0, additionalCosts = 0, sellingCostPct, years, incomeTaxRate, standardDeduction, includeTaxBenefit }) {
  ...
  const initialCashNeeded = downPayment + buyingClosingCosts + transferTax + recordationTax + additionalCosts + renovations;
  ...
  return { rows, downPayment, buyingClosingCosts, transferTax, recordationTax, additionalCosts, initialCashNeeded, loanAmount, mortgage, monthlyTaxes, monthlyInsurance, monthlyMaintenance, monthlyOwnershipCost, totalPrincipalPaid, totalInterestPaid, totalTaxBenefit, winner, gap: Math.abs(last.buyingNetWorth - last.rentingNetWorth), buyingNetWorth: last.buyingNetWorth, rentingNetWorth: last.rentingNetWorth, equityAfterSale: last.equityAfterSale, buyerSurplusPortfolio: last.buyerSurplusPortfolio, endingHomeValue: last.homeValue, endingLoanBalance: last.loanBalance };
}

function RentVsBuyCalculator() {
  const [homePrice, setHomePrice] = useCalculatorState("rent-vs-buy", "homePrice", 1000000);
  const [downPaymentPct, setDownPaymentPct] = useCalculatorState("rent-vs-buy", "downPaymentPct", 20);
  const [rate, setRate] = useCalculatorState("rent-vs-buy", "rate", 5.5);
  const [loanYears, setLoanYears] = useCalculatorState("rent-vs-buy", "loanYears", 30);
  const [rent, setRent] = useCalculatorState("rent-vs-buy", "rent", 5000);
  const [monthlyCashBeforeHousing, setMonthlyCashBeforeHousing] = useCalculatorState("rent-vs-buy", "monthlyCashBeforeHousing", 10000);
  const [appreciation, setAppreciation] = useCalculatorState("rent-vs-buy", "appreciation", 4);
  const [marketReturn, setMarketReturn] = useCalculatorState("rent-vs-buy", "marketReturn", 10);
  const [rentInflation, setRentInflation] = useCalculatorState("rent-vs-buy", "rentInflation", 2);
  const [propertyTaxPct, setPropertyTaxPct] = useCalculatorState("rent-vs-buy", "propertyTaxPct", 1.1);
  const [annualInsurance, setAnnualInsurance] = useCalculatorState("rent-vs-buy", "annualInsurance", 3600);
  const [maintenancePct, setMaintenancePct] = useCalculatorState("rent-vs-buy", "maintenancePct", 1);
  const [renovations, setRenovations] = useCalculatorState("rent-vs-buy", "renovations", 0);
  const [buyingClosingCostPct, setBuyingClosingCostPct] = useCalculatorState("rent-vs-buy", "buyingClosingCostPct", 2.5);
  const [transferTaxPct, setTransferTaxPct] = useCalculatorState("rent-vs-buy", "transferTaxPct", 0);
  const [recordationTaxPct, setRecordationTaxPct] = useCalculatorState("rent-vs-buy", "recordationTaxPct", 0);
  const [additionalCosts, setAdditionalCosts] = useCalculatorState("rent-vs-buy", "additionalCosts", 0);
  const [sellingCostPct, setSellingCostPct] = useCalculatorState("rent-vs-buy", "sellingCostPct", 6);
  const [years, setYears] = useCalculatorState("rent-vs-buy", "years", 30);
  const [incomeTaxRate, setIncomeTaxRate] = useCalculatorState("rent-vs-buy", "incomeTaxRate", 35);
  const [standardDeduction, setStandardDeduction] = useCalculatorState("rent-vs-buy", "standardDeduction", 30000);
  const [includeTaxBenefit, setIncludeTaxBenefit] = useCalculatorState("rent-vs-buy", "includeTaxBenefit", true);
  const [showEquityLine, setShowEquityLine] = useCalculatorState("rent-vs-buy", "showEquityLine", false);
  const [showHomeValueLine, setShowHomeValueLine] = useCalculatorState("rent-vs-buy", "showHomeValueLine", false);
  const [showCashFlowLine, setShowCashFlowLine] = useCalculatorState("rent-vs-buy", "showCashFlowLine", false);
  const [showTable, setShowTable] = useCalculatorState("rent-vs-buy", "showTable", false);
  const result = useMemo(() => calculateRentVsBuyScenario({ homePrice, downPaymentPct, rate, loanYears, rent, monthlyCashBeforeHousing, appreciation, marketReturn, rentInflation, propertyTaxPct, annualInsurance, maintenancePct, renovations, buyingClosingCostPct, transferTaxPct, recordationTaxPct, additionalCosts, sellingCostPct, years, incomeTaxRate, standardDeduction, includeTaxBenefit }), [homePrice, downPaymentPct, rate, loanYears, rent, monthlyCashBeforeHousing, appreciation, marketReturn, rentInflation, propertyTaxPct, annualInsurance, maintenancePct, renovations, buyingClosingCostPct, transferTaxPct, recordationTaxPct, additionalCosts, sellingCostPct, years, incomeTaxRate, standardDeduction, includeTaxBenefit]);
...
function RentalPropertyCalculator({ title = "Rental Property vs. Market Investment Calculator", description = "A cleaner grouped-layout version of the rental property calculator." }) {
  const [purchasePrice, setPurchasePrice] = useCalculatorState("rental-property-2", "purchasePrice", 750000);
  const [monthlyRent, setMonthlyRent] = useCalculatorState("rental-property-2", "monthlyRent", 4500);
  const [downPaymentPct, setDownPaymentPct] = useCalculatorState("rental-property-2", "downPaymentPct", 25);
  const [rate, setRate] = useCalculatorState("rental-property-2", "rate", 6.75);
  const [appreciation, setAppreciation] = useCalculatorState("rental-property-2", "appreciation", 4);
  const [marketReturn, setMarketReturn] = useCalculatorState("rental-property-2", "marketReturn", 10);
  const [rentGrowth, setRentGrowth] = useCalculatorState("rental-property-2", "rentGrowth", 2);
  const [vacancyPct, setVacancyPct] = useCalculatorState("rental-property-2", "vacancyPct", 5);
  const [propertyTaxPct, setPropertyTaxPct] = useCalculatorState("rental-property-2", "propertyTaxPct", 1.1);
  const [annualInsurance, setAnnualInsurance] = useCalculatorState("rental-property-2", "annualInsurance", 3000);
  const [maintenancePct, setMaintenancePct] = useCalculatorState("rental-property-2", "maintenancePct", 1);
  const [managementPct, setManagementPct] = useCalculatorState("rental-property-2", "managementPct", 8);
  const [closingCostPct, setClosingCostPct] = useCalculatorState("rental-property-2", "closingCostPct", 2.5);
  const [sellingCostPct, setSellingCostPct] = useCalculatorState("rental-property-2", "sellingCostPct", 6);
  const [years, setYears] = useCalculatorState("rental-property-2", "years", 30);
  const [cashFlowStrategy, setCashFlowStrategy] = useCalculatorState("rental-property-2", "cashFlowStrategy", "reinvest");
  const [reinvestLossCoverage, setReinvestLossCoverage] = useCalculatorState("rental-property-2", "reinvestLossCoverage", true);
...
function AutoCostCalculator() {
  const [yearsOwned, setYearsOwned] = useCalculatorState("auto-cost", "yearsOwned", 10);
  const [marketReturn, setMarketReturn] = useCalculatorState("auto-cost", "marketReturn", 10);
  const [equalizedPrice, setEqualizedPrice] = useCalculatorState("auto-cost", "equalizedPrice", 45000);
  const [usedPrice, setUsedPrice] = useCalculatorState("auto-cost", "usedPrice", 28000);
  const [usedDownPayment, setUsedDownPayment] = useCalculatorState("auto-cost", "usedDownPayment", 20);
  const [usedTradeInValue, setUsedTradeInValue] = useCalculatorState("auto-cost", "usedTradeInValue", 0);
  const [usedRate, setUsedRate] = useCalculatorState("auto-cost", "usedRate", 6.5);
  const [usedLoanMonths, setUsedLoanMonths] = useCalculatorState("auto-cost", "usedLoanMonths", 60);
  const [usedDepreciation, setUsedDepreciation] = useCalculatorState("auto-cost", "usedDepreciation", 10);
  const [usedInsurance, setUsedInsurance] = useCalculatorState("auto-cost", "usedInsurance", 1800);
  const [usedMaintenance, setUsedMaintenance] = useCalculatorState("auto-cost", "usedMaintenance", 1600);
  const [usedRegistration, setUsedRegistration] = useCalculatorState("auto-cost", "usedRegistration", 350);
  const [usedFuel, setUsedFuel] = useCalculatorState("auto-cost", "usedFuel", 2400);
  const [newPrice, setNewPrice] = useCalculatorState("auto-cost", "newPrice", 48000);
  const [newDownPayment, setNewDownPayment] = useCalculatorState("auto-cost", "newDownPayment", 20);
  const [newTradeInValue, setNewTradeInValue] = useCalculatorState("auto-cost", "newTradeInValue", 0);
  const [newRate, setNewRate] = useCalculatorState("auto-cost", "newRate", 6.5);
  const [newLoanMonths, setNewLoanMonths] = useCalculatorState("auto-cost", "newLoanMonths", 60);
  const [newDepreciation, setNewDepreciation] = useCalculatorState("auto-cost", "newDepreciation", 15);
  const [newInsurance, setNewInsurance] = useCalculatorState("auto-cost", "newInsurance", 2400);
  const [newMaintenance, setNewMaintenance] = useCalculatorState("auto-cost", "newMaintenance", 900);
  const [newRegistration, setNewRegistration] = useCalculatorState("auto-cost", "newRegistration", 550);
  const [newFuel, setNewFuel] = useCalculatorState("auto-cost", "newFuel", 2400);
  const [leaseCarValue, setLeaseCarValue] = useCalculatorState("auto-cost", "leaseCarValue", 45000);
  const [leaseDueAtSigning, setLeaseDueAtSigning] = useCalculatorState("auto-cost", "leaseDueAtSigning", 4000);
  const [leaseMonthlyPayment, setLeaseMonthlyPayment] = useCalculatorState("auto-cost", "leaseMonthlyPayment", 550);
  const [leaseInsurance, setLeaseInsurance] = useCalculatorState("auto-cost", "leaseInsurance", 2400);
  const [leaseMaintenance, setLeaseMaintenance] = useCalculatorState("auto-cost", "leaseMaintenance", 500);
  const [leaseRegistration, setLeaseRegistration] = useCalculatorState("auto-cost", "leaseRegistration", 550);
  const [leaseFuel, setLeaseFuel] = useCalculatorState("auto-cost", "leaseFuel", 2400);
  const [leaseDispositionFee, setLeaseDispositionFee] = useCalculatorState("auto-cost", "leaseDispositionFee", 500);
  const [leaseResidualPct, setLeaseResidualPct] = useCalculatorState("auto-cost", "leaseResidualPct", 55);
  const [leasePostBuyoutDepreciation, setLeasePostBuyoutDepreciation] = useCalculatorState("auto-cost", "leasePostBuyoutDepreciation", 10);
  const [leaseBuyoutAtEnd, setLeaseBuyoutAtEnd] = useCalculatorState("auto-cost", "leaseBuyoutAtEnd", false);
...
function calculateHomeValueScenario({ homePrice, salePrice, downPaymentPct, rate, loanTerm, yearsHeld, monthlyRent, marketReturn, rentInflation, propertyTaxPct, insuranceAnnual, maintenancePct, renovations, closingCostPct, transferTaxPct = 0, recordationTaxPct = 0, additionalCosts = 0, sellingCostPct }) {
  const downPayment = homePrice * (downPaymentPct / 100);
  const buyingClosingCosts = homePrice * (closingCostPct / 100);
  const transferTax = homePrice * (transferTaxPct / 100);
  const recordationTax = homePrice * (recordationTaxPct / 100);
  const initialCash = downPayment + buyingClosingCosts + transferTax + recordationTax + additionalCosts + renovations;
  const loanAmount = Math.max(0, homePrice - downPayment);
  ...
  return { rows, downPayment, buyingClosingCosts, transferTax, recordationTax, additionalCosts, initialCash, loanAmount, mortgage, monthlyPropertyTax, monthlyInsurance, monthlyMaintenance, monthlyOwnershipCost, sellingCosts: last.homeValue * (sellingCostPct / 100), totalTransactionAndOwnershipCosts: buyingClosingCosts + transferTax + recordationTax + additionalCosts + last.homeValue * (sellingCostPct / 100) + (monthlyPropertyTax + monthlyInsurance + monthlyMaintenance) * 12 + renovations, totalPrincipalPaid, totalInterestPaid, netSaleProceeds: last.equityAfterSale, mortgageBalance: last.mortgageBalance, winner, gap: Math.abs(last.homeValueResult - last.marketInvestmentResult), houseResult: last.homeValueResult, marketResult: last.marketInvestmentResult, homeValue: last.homeValue, impliedAnnualAppreciation: impliedAnnualAppreciation * 100 };
}
function HomeValueCalculator() {
  const [homePrice, setHomePrice] = useCalculatorState("home-value-vs-market", "homePrice", 1800000);
  const [salePrice, setSalePrice] = useCalculatorState("home-value-vs-market", "salePrice", 3000000);
  const [downPaymentPct, setDownPaymentPct] = useCalculatorState("home-value-vs-market", "downPaymentPct", 20);
  const [rate, setRate] = useCalculatorState("home-value-vs-market", "rate", 5.65);
  const [loanTerm, setLoanTerm] = useCalculatorState("home-value-vs-market", "loanTerm", 30);
  const [yearsHeld, setYearsHeld] = useCalculatorState("home-value-vs-market", "yearsHeld", 30);
  const [monthlyRent, setMonthlyRent] = useCalculatorState("home-value-vs-market", "monthlyRent", 6200);
  const [marketReturn, setMarketReturn] = useCalculatorState("home-value-vs-market", "marketReturn", 10);
  const [rentInflation, setRentInflation] = useCalculatorState("home-value-vs-market", "rentInflation", 2);
  const [propertyTaxPct, setPropertyTaxPct] = useCalculatorState("home-value-vs-market", "propertyTaxPct", 1.1);
  const [insuranceAnnual, setInsuranceAnnual] = useCalculatorState("home-value-vs-market", "insuranceAnnual", 3600);
  const [maintenancePct, setMaintenancePct] = useCalculatorState("home-value-vs-market", "maintenancePct", 1);
  const [renovations, setRenovations] = useCalculatorState("home-value-vs-market", "renovations", 0);
  const [closingCostPct, setClosingCostPct] = useCalculatorState("home-value-vs-market", "closingCostPct", 2.5);
  const [transferTaxPct, setTransferTaxPct] = useCalculatorState("home-value-vs-market", "transferTaxPct", 0);
  const [recordationTaxPct, setRecordationTaxPct] = useCalculatorState("home-value-vs-market", "recordationTaxPct", 0);
  const [additionalCosts, setAdditionalCosts] = useCalculatorState("home-value-vs-market", "additionalCosts", 0);
  const [sellingCostPct, setSellingCostPct] = useCalculatorState("home-value-vs-market", "sellingCostPct", 6);
  const result = useMemo(() => calculateHomeValueScenario({ homePrice, salePrice, downPaymentPct, rate, loanTerm, yearsHeld, monthlyRent, marketReturn, rentInflation, propertyTaxPct, insuranceAnnual, maintenancePct, renovations, closingCostPct, transferTaxPct, recordationTaxPct, additionalCosts, sellingCostPct }), [homePrice, salePrice, downPaymentPct, rate, loanTerm, yearsHeld, monthlyRent, marketReturn, rentInflation, propertyTaxPct, insuranceAnnual, maintenancePct, renovations, closingCostPct, transferTaxPct, recordationTaxPct, additionalCosts, sellingCostPct]);
...
function CollegeSavingsCalculator() {
  const [childCount, setChildCount] = useCalculatorState("college-savings", "childCount", 2);
  const [children, setChildren] = useCalculatorState("college-savings", "children", () => makeCollegeChildren(2));
  const [annualTuition, setAnnualTuition] = useCalculatorState("college-savings", "annualTuition", 35000);
  const [annualBoard, setAnnualBoard] = useCalculatorState("college-savings", "annualBoard", 18000);
  const [startingAmount, setStartingAmount] = useCalculatorState("college-savings", "startingAmount", 25000);
  const [monthlyContribution, setMonthlyContribution] = useCalculatorState("college-savings", "monthlyContribution", 750);
  const [marketReturn, setMarketReturn] = useCalculatorState("college-savings", "marketReturn", 10);
  const [inflation, setInflation] = useCalculatorState("college-savings", "inflation", 4);
  const [showTable, setShowTable] = useCalculatorState("college-savings", "showTable", false);
...
function GenerationalSavingsCalculator() {
  const [childCount, setChildCount] = useCalculatorState("generational-savings", "childCount", 2);
  const [children, setChildren] = useCalculatorState("generational-savings", "children", () => makeGenerationalChildren(2));
  const [startingAmount, setStartingAmount] = useCalculatorState("generational-savings", "startingAmount", 10000);
  const [monthlyContribution, setMonthlyContribution] = useCalculatorState("generational-savings", "monthlyContribution", 1000);
  const [marketReturn, setMarketReturn] = useCalculatorState("generational-savings", "marketReturn", 10);
  const [inflation, setInflation] = useCalculatorState("generational-savings", "inflation", 3);
  const [carCost, setCarCost] = useCalculatorState("generational-savings", "carCost", 30000);
  const [annualTuition, setAnnualTuition] = useCalculatorState("generational-savings", "annualTuition", 35000);
  const [annualBoard, setAnnualBoard] = useCalculatorState("generational-savings", "annualBoard", 18000);
  const [annualPostgradTuition, setAnnualPostgradTuition] = useCalculatorState("generational-savings", "annualPostgradTuition", 60000);
  const [annualPostgradBoard, setAnnualPostgradBoard] = useCalculatorState("generational-savings", "annualPostgradBoard", 24000);
  const [postgradYears, setPostgradYears] = useCalculatorState("generational-savings", "postgradYears", 2);
  const [downPayment, setDownPayment] = useCalculatorState("generational-savings", "downPayment", 150000);
...
function calculateMedicalCostsScenario({ startingAge, lifeExpectancy, yearlyInvestedAmount, employeeCostPerYear, healthStatus, marketReturn, inflation }) {
  ...
  return { rows, years, endingBalance, cumulativeInvested, cumulativeEmployeeCosts, cumulativeCashCosts, shortfall, firstShortfallAge };
}
...
function MedicalCostsCalculator() {
  ...
  const chartData = result.rows.map((row) => ({ ...row, "Invested Balance": row.investedBalance, "Cumulative Invested": row.cumulativeInvested, "Cumulative Employee Costs": row.cumulativeEmployeeCosts, "Cumulative Medical Costs": row.cumulativeCashCosts, Shortfall: row.shortfall }));
  ...
  const totalInvestmentProfit = Math.max(0, result.endingBalance + result.cumulativeCashCosts - result.cumulativeInvested);
  const fundedDetail = result.shortfall > 0 ? `${formatCompactMoney(result.shortfall)} uncovered` : "Cash costs covered";
  const shortfallDetail = result.firstShortfallAge === null ? "No shortfall" : `Starts at age ${result.firstShortfallAge}`;
  return (
    <CalculatorFrame title="Medical Costs Calculator" badge="Beta" description="Compare investing cash directly with drawing from it for age- and health-status-based medical costs later." exportData={{ columns: MEDICAL_COST_EXPORT_COLUMNS, rows: result.rows }}>
      <div className="space-y-4">
        <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-5">
          ...
          <SmallStat label="Shortfall" value={formatCompactMoney(result.shortfall)} detail={shortfallDetail} tone="red" />
          <SmallStat label="Total investment profit" value={formatCompactMoney(totalInvestmentProfit)} detail={`Cumulative invested: ${formatCompactMoney(result.cumulativeInvested)}`} tone="blue" />
          <SmallStat label="Total medical costs" value={formatCompactMoney(result.cumulativeCashCosts)} tone="magenta" />
        ...
        <div className="grid gap-5 xl:grid-cols-[430px_1fr]">
          ...
          <Card className="flex min-h-[560px] flex-col">
            ...
            <div className="min-h-[440px] flex-1"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="age" tickLine={false} axisLine={false} /><YAxis tickFormatter={formatCompactMoney} tickLine={false} axisLine={false} width={72} /><Tooltip content={<ChartTooltip />} /><Legend /><Line type="monotone" dataKey="Invested Balance" stroke="#059669" strokeWidth={3} dot={false} /><Line type="monotone" dataKey="Cumulative Invested" stroke="#0284c7" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="Cumulative Employee Costs" stroke="#f59e0b" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="Cumulative Medical Costs" stroke="#d946ef" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="Shortfall" stroke="#dc2626" strokeWidth={2} dot={false} strokeDasharray="3 4" /></LineChart></ResponsiveContainer></div>
          </Card>
        </div>
        <div className="grid gap-5 lg:grid-cols-2">
          <Card><SectionTitle icon={TrendingUpIcon} title="Investment assumptions" /><div className="space-y-4"><MarketReturnPicker value={marketReturn} onChange={setMarketReturn} /><PercentInput label="Medical cost inflation" value={inflation} onChange={setInflation} min={0} max={15} helperText="Inflates both the yearly invested amount and selected health-profile medical costs." /></div></Card>
          <Card><SectionTitle icon={CalculatorIcon} title="Annual medical cost table" /><div className="max-h-[420px] overflow-y-auto rounded-2xl border border-neutral-200"><table className="w-full min-w-[520px] table-fixed border-separate border-spacing-0 text-left text-xs"><thead className="sticky top-0 bg-neutral-50 text-neutral-500"><tr><th className="border-b border-neutral-200 px-3 py-2">Age</th><th className="border-b border-neutral-200 px-3 py-2 text-right">Healthy</th><th className="border-b border-neutral-200 px-3 py-2 text-right">Unhealthy</th><th className="border-b border-neutral-200 px-3 py-2 text-right">Very unhealthy</th></tr></thead><tbody>{MEDICAL_ANNUAL_COSTS_BY_AGE.map((row) => <tr key={row.age}><td className="border-b border-neutral-100 px-3 py-2 font-semibold">{row.age}</td><td className="border-b border-neutral-100 px-3 py-2 text-right">{formatMoney(row.healthy)}</td><td className="border-b border-neutral-100 px-3 py-2 text-right">{formatMoney(row.unhealthy)}</td><td className="border-b border-neutral-100 px-3 py-2 text-right">{formatMoney(row["very-unhealthy"])}</td></tr>)}</tbody></table></div><p className="mt-3 text-xs leading-5 text-neutral-500">Figures are annual estimates in 2024 dollars.</p></Card>
        </div>
        <Card>
          <SectionTitle icon={BarChartIcon} title="Women and Men annual Medicaid cost" subtitle="Estimated annual Medicaid costs by age from men and women." />
          <div className="h-[460px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={sexCostChartData} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}><CartesianGrid strokeDasharray="3 3" /><ReferenceArea x1={20} x2={45} fill="#d4d4d4" fillOpacity={0.22} label={{ value: "Reproductive years", position: "insideTop", fill: "#737373", fontSize: 11 }} /><ReferenceArea x1={83} x2={100} fill="#d4d4d4" fillOpacity={0.22} label={{ value: "Nursing home years", position: "insideTop", fill: "#737373", fontSize: 11 }} /><ReferenceLine x={65} stroke="#737373" strokeDasharray="4 4" label={{ value: "Medicare takes over", position: "insideTopRight", fill: "#737373", fontSize: 11 }} /><XAxis dataKey="age" type="number" domain={[20, 100]} allowDecimals={false} tickLine={false} axisLine={false} /><YAxis tickFormatter={formatCompactMoney} tickLine={false} axisLine={false} width={72} /><Tooltip content={<ChartTooltip />} /><Legend /><Line type="monotone" dataKey="Women" stroke="#D85A30" strokeWidth={3} dot={false} /><Line type="monotone" dataKey="Men" stroke="#378ADD" strokeWidth={3} dot={false} strokeDasharray="6 3" /></LineChart></ResponsiveContainer></div>
...
const pageCopy = {
  ...
  retirement: {
    ...
    sections: [
      { title: "How to interpret the result", body: "The retirement balance is the estimated portfolio at the selected retirement age. The retirement runway shows how long the modeled balance supports inflated spending after estimated federal tax." },
      { title: "What to verify", body: "Confirm tax filing status, account mix, withdrawal order, required minimum distributions, Social Security, pensions, health care, state taxes, and actual investment fees before relying on the estimate." },
    ],
  },
  "college-savings": {
